...
MODULE_DESCRIPTION("KERNEL SYNCRONIZITION FOR MULTIPLEDEVICE");
...
static ssize_t char_write(struct file *filp, const char *Ubuf, size_t len, loff_t *offp) 
{
    ...
    down(&my_sema) ;
    if((copy_from_user(kernel_buffer,Ubuf,len))<0)
    {
        pr_err("caanot copy from the user");
    }
...
static ssize_t char_read(struct file *filp, char __user *Ubuf, size_t len, loff_t *offp) 
{
    ...
    if((copy_to_user(Ubuf,kernel_buffer,len))<0)
    {
        pr_err("cannot copy to the user");
    }
     up(&my_sema); 
...
static int __init charDev_fops_init(void) 
{
    ...
    if(alloc_chrdev_region(&mydev,0,2,"charDev_fops") < 0) {
